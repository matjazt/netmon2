...
public interface DeviceRepository extends JpaRepository<DeviceEntity, Long> {
    ...
    @Query("select new com.matjazt.netmon2.dto.response.DeviceResponseDto(" +
            "d.id, d.network.id, d.name, d.macAddress, d.ipAddress, d.online, d.lastSeen, d.deviceOperationMode) " +
            "from DeviceEntity d")
    List<DeviceResponseDto> findAllSummaries();

    @Query("select new com.matjazt.netmon2.dto.response.DeviceResponseDto(" +
            "d.id, d.network.id, d.name, d.macAddress, d.ipAddress, d.online, d.lastSeen, d.deviceOperationMode) " +
            "from DeviceEntity d where d.network.id = :networkId")
    List<DeviceResponseDto> findSummariesByNetworkId(@Param("networkId") Long networkId);

    @Query("select new com.matjazt.netmon2.dto.response.DeviceResponseDto(" +
            "d.id, d.network.id, d.name, d.macAddress, d.ipAddress, d.online, d.lastSeen, d.deviceOperationMode) " +
            "from DeviceEntity d where d.network.id = :networkId and d.online = true")
    List<DeviceResponseDto> findOnlineSummariesByNetworkId(@Param("networkId") Long networkId);

    @Query(value = "select new com.matjazt.netmon2.dto.response.DeviceResponseDto(" +
            "d.id, d.network.id, d.name, d.macAddress, d.ipAddress, d.online, d.lastSeen, d.deviceOperationMode) " +
            "from DeviceEntity d", countQuery = "select count(d) from DeviceEntity d")
    Page<DeviceResponseDto> findAllSummaries(Pageable pageable);

    @Query(value = "select new com.matjazt.netmon2.dto.response.DeviceResponseDto(" +
            "d.id, d.network.id, d.name, d.macAddress, d.ipAddress, d.online, d.lastSeen, d.deviceOperationMode) " +
            "from DeviceEntity d where d.network.id = :networkId", countQuery = "select count(d) from DeviceEntity d where d.network.id = :networkId")
    Page<DeviceResponseDto> findSummariesByNetworkId(@Param("networkId") Long networkId, Pageable pageable);
